...
public interface PetRepository extends JpaRepository<Pet, Long> {
  ...
  Page<Pet> getAllByType_Id(Pageable pageable, Long typeId);

  Page<Pet> getAllByBreed_Id(Pageable pageable, Long breedId);

  @Query("SELECT p FROM Pet p WHERE p.type.id IN :typeIds AND p.active = true")
  Page<Pet> findByTypes(
      Pageable pageable, @Param("typeIds") List<Long> typeIds);

  @Query(
      value =
          "SELECT * FROM Pet p WHERE LOWER(REGEXP_REPLACE(p.name, '[^a-zA-Z0-9]', '')) LIKE LOWER(CONCAT('%', :name, '%'))",
      nativeQuery = true)
  List<Pet> findAllByNameContainingIgnoreCase();

  Page<Pet> getAllByUser_IdAndActiveTrue(Pageable pageable, Long userId);
